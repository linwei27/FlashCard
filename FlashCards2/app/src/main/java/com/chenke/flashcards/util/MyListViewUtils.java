...
public class MyListViewUtils extends ListView implements AbsListView.OnScrollListener{
    ...
    private void Init(Context context) {
        //拿到头布局文件xml
        headview=LinearLayout.inflate(context, R.layout.activity_head, null);
        headtxt= headview.findViewById(R.id.headtxt);
        headtime= headview.findViewById(R.id.timetxt);
        progressBar= headview.findViewById(R.id.headprogress);
        headtime.setText("上次更新时间:"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis()));

        //拿到尾布局文件
        bottomview=LinearLayout.inflate(context, R.layout.activity_bottom, null);
        //测量尾文件高度
